...
interface HomePageSliceProps {
  notes: Note[];
  query: string;
  searchSlot: React.ReactNode;
}

export function HomePageSlice({ notes, query, searchSlot }: HomePageSliceProps) {
  const state = getHomeDiscoveryState(notes, query);
  ...
  const hero = (
    ...
    >
      <div className="page-wrap" style={{ position: 'relative' }}>
        ...
        >
          {notes.length} notes on software engineering, books, productivity, and more.{' '}
        ...
        <div style={{ maxWidth: 540 }}>{searchSlot}</div>
      </div>
    </section>
  );
  ...
  const defaultSections = (
    <div className="page-wrap" style={{ paddingTop: 48, paddingBottom: 'var(--space-8)' }}>
      {state.sortedCategories.map((category) => {
        const categoryNotes = state.grouped[category] ?? [];
        const visibleNotes = categoryNotes.slice(0, MAX_CARDS_PER_SECTION);
        const hasMore = categoryNotes.length > MAX_CARDS_PER_SECTION;
            ...
            >
              ...
              {hasMore && (
                <Link
                  ...
                  aria-label={`View all ${categoryLabel(category)} notes`}
                >
                  VIEW ALL →
                </Link>
              )}
            ...
            >
              {visibleNotes.map((note, index) => (
                <garden-card
                  ...
                  style={
                    {
                      '--card-rotate': index % 2 === 0 ? '-0.6deg' : '0.7deg',
  ...
  const resultsState = (
    ...
    >
      ...
      >
        ...
        <span
          aria-label={`${state.results.length} search results`}
          style={{
            fontFamily: 'var(--font-stamp)',
            fontSize: 11,
            background: 'var(--zine-yellow)',
      ...
      {state.mode === 'no-results' ? (
        ...
        >
          No notes found for <strong>{state.query}</strong>. Try a different search term.
        </div>
      ) : (
        <div
          style={{
  ...
  return (
    <main>
      {marqueeBanner}
      {hero}
      {state.mode === 'default' ? defaultSections : resultsState}
    </main>
  );
}
