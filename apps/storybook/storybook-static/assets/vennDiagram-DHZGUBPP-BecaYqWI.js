...
const Dt = 1e-10;
function ft(t, n, s) {
  return Math.min(t, n) * Math.min(t, n) * Math.PI <= s + Dt
    ? Math.abs(t - n)
...
function ye(t, n) {
  ...
  for (const y of t) {
    ...
    const m = y.sets[0],
      x = y.sets[1];
    (y.size + Dt >= Math.min(e[m].size, e[x].size) && (d = 0),
...
function be(t = {}) {
  ...
  function l(p) {
    ...
    const R = U.selectAll('.venn-area').data(h, (I) => I.sets),
      O = R.enter()
        .append('g')
        .attr(
          'class',
    ...
    z && typeof T.transition == 'function'
      ? ((T = D(p)), T.selectAll('path').attrTween('d', A))
    ...
    return (
      ...
      { circles: k, textCentres: N, nodes: R, enter: O, update: T, exit: C }
    );
  }
  return (
    (l.wrap = function (p) {
        ...
        for (var T, F, C, X, I = {}, L, G, Z, H; ; ) {
          if (
            ...
            this.defaultActions[F]
              ...
              : ((T === null || typeof T > 'u') && (T = D()), (C = j[F] && j[F][T])),
...
var Be = S((t, n, s, e) => {
  ...
  (l.selectAll('.venn-circle').each(function (A, R) {
    ...
    const O = rt(this),
      V = $([...A.sets].sort()),
      D = x.get(V),
      T = (D == null ? void 0 : D.fill) || a[R % a.length] || o.primaryColor;
    O.classed(`venn-set-${R % 8}`, !0);
    const F = (D == null ? void 0 : D['fill-opacity']) ?? 0.1,
      C = (D == null ? void 0 : D.stroke) || T,
      X = (D == null ? void 0 : D['stroke-width']) || `${5 * w}`;
    if (u && h) {
      const G = k.get(V);
    ...
    u && h
      ? l.selectAll('.venn-intersection').each(function (A) {
          ...
          const R = rt(this),
            P = $([...A.sets].sort()),
            V = x.get(P),
            D = V == null ? void 0 : V.fill;
          if (D) {
            const F = R.select('path'),
              C = F.attr('d');
            if (C) {
              const X = h.path(C, {
                  roughness: 0.7,
...
var qe = { draw: Be },
  Ke = { parser: we, db: je, renderer: qe, styles: Ve };
