...
import { s as Ce } from './chunk-EDXVE4YY-CXXKLgPc.js';
import { p as Te } from './chunk-4BX2VUAB-DNt9yf9l.js';
import { p as Le } from './wardley-RL74JXVD-DbzsJyh7.js';
...
function Ne(t, a) {
  for (var l = this, n = [l], r, s, d = -1; (l = n.pop()); )
    if ((t.call(a, l, ++d, this), (r = l.children)))
      for (s = r.length - 1; s >= 0; --s) n.push(r[s]);
  return this;
}
function Me(t, a) {
...
function Je(t, a, l, n, r, s) {
  for (
    ...
    while (!p && b < i);
    for (
      m = M = p, E = Math.max(S / x, x / S) / (v * t), V = p * p * E, z = Math.max(M / V, V / m);
      b < i;
      ++b
    ) {
      if (
    ...
    (d.push((g = { value: p, dice: x < S, children: h.slice(u, b) })),
      g.dice
        ? je(g, l, n, r, v ? (n += (S * p) / v) : s)
        : Ue(g, l, n, v ? (l += (x * p) / v) : r, s),
      (v -= p),
      (u = b));
  }
  return d;
  ...
  return (
    ...
    (u.paddingOuter = function (i) {
      return arguments.length
        ? u.paddingTop(i).paddingRight(i).paddingBottom(i).paddingLeft(i)
        : u.paddingTop();
    }),
        ...
        .each(function (e) {
          ...
          if (y.getComputedTextLength() > F) {
            ...
            for (; L.length > 0; ) {
              ...
              if ((o.text(L + T), y.getComputedTextLength() <= F)) break;
            }
          }
        }),
      s.showValues !== !1 &&
      ...
      Y.append('text')
        ...
        .each(function (e) {
          ...
          const _ = 8,
            ...
            k = 6,
            H = 2;
          for (; $.getComputedTextLength() > D && y > _; ) (y--, o.style('font-size', `${y}px`));
          let I = Math.max(k, Math.min(T, Math.round(y * L))),
            Z = y + H + I;
          for (
            ;
            Z > F &&
            y > _ &&
            (y--, (I = Math.max(k, Math.min(T, Math.round(y * L)))), !(I < k && y === _));
          )
            (o.style('font-size', `${y}px`), (Z = y + H + I));
          (o.style('font-size', `${y}px`),
            ($.getComputedTextLength() > D || y < _ || F < y) && o.style('display', 'none'));
        }),
      s.showValues !== !1 &&
        Y.append('text')
          ...
          .each(function (o) {
            ...
            const A = parseFloat($.style('font-size')),
              ...
              y = 6,
              _ = 2,
              T = Math.max(y, Math.min(D, Math.round(A * F)));
            ...
            const H = o.x1 - o.x0,
              ce = o.y1 - o.y0 - 4,
              de = H - 2 * 4;
            f.node().getComputedTextLength() > de || k + T > ce || T < y
              ? f.style('display', 'none')
