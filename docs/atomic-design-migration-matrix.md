...
- **active shared** — approved shared UI that belongs in the atomic Storybook taxonomy (`Atoms/*`, `Molecules/*`, `Organisms/*`, `Templates/*`)
- **undecided** — shared-package artifact kept visible for review, but not approved for new shared adoption until real route reuse is proven
- **legacy** — backward-compatibility export kept temporarily with an explicit replacement path and no new usage
...
| `garden-tape`       | `packages/components/src/garden-tape/`       | Atom         | `undecided`           | Human approval needed. Decorative primitive with Storybook-only usage; use route-local decoration instead of expanding shared usage until the scrapbook accent is proven on more than one real route.        |
...
| `garden-banner`     | `packages/components/src/garden-banner/`     | Organism     | `undecided`           | Human approval needed. The live home page currently reimplements the marquee inline instead of using this shared component; keep marquee/banner composition route-local until a second real route proves it. |
| `garden-sheet`      | `packages/components/src/garden-sheet/`      | Organism     | `undecided`           | Human approval needed. Visually overlaps with the home page’s pinned-paper sections, but no live route consumes it yet; keep pinned-paper wrappers route-local until reuse is proven.                        |
...
| `my-heading`        | `packages/components/src/my-heading/`        | Atom         | `legacy`              | Legacy-only. Backward-compat export with no live-site usage; replace with semantic headings or shared template headings and do not expand it inside the new taxonomy.                                        |
